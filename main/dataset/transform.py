...
def fit_ortho_param(joints3d: np.ndarray, joints2d: np.ndarray) -> np.ndarray:
    ...
    pad1 = 1 - pad2
    jM = np.concatenate([joints3d_xy, pad1, pad2], axis=1)           
    jMT = jM.transpose()           
...
def batch_xyz2uvd(
    ...
    inp_res = torch.Tensor(inp_res).to(xyz.device)               
    batch_size = xyz.shape[0]
    if ref_bone_len is None:
        ref_bone_len = torch.ones((batch_size, 1)).to(xyz.device)                 
    ...
    if camera_mode == "persp":
        ...
        z = xyz[:, :, 2]                   
        xy = xyz[:, :, :2]                      
        xy_ = xy / z.unsqueeze(-1).expand_as(xy)                      
        root_joint_z = root_joint[:, -1].unsqueeze(-1)                 
        z_ = (z - root_joint_z.expand_as(z)) / ref_bone_len.expand_as(z)                   
        ...
        camparam = torch.cat((fx, fy, cx, cy), dim=1)                 
        camparam = camparam.unsqueeze(1).expand(-1, xyz.shape[1], -1)                      
        uv = (xy_ * camparam[:, :, :2]) + camparam[:, :, 2:4]                      

                                  
        uv = torch.einsum("bij, j->bij", uv, 1.0 / inp_res)                               
        d = z_ / depth_range + 0.5                            
        uvd = torch.cat((uv, d.unsqueeze(-1)), -1)                      
    elif camera_mode == "ortho":
        assert intr.dim() == 2, f"Unexpected dim, expect intr has shape (B, 3), got {intr.shape}"
                       
        xyz = xyz - root_joint.unsqueeze(1)                      

        xy = xyz[:, :, :2]                      
        z = xyz[:, :, 2]                   
        z_ = z / ref_bone_len.expand_as(z)                   
        d = z_ / depth_range + 0.5                            

        scale = intr[:, :1].unsqueeze(1)                    
        shift = intr[:, 1:].unsqueeze(1)                    
        uv = xy * scale + shift                                     
        uv = torch.einsum("bij,j->bij", uv, 1.0 / inp_res)                                     
        uvd = torch.cat((uv, d.unsqueeze(-1)), -1)                      
...
def batch_uvd2xyz(
    ...
    uv = torch.einsum("bij,j->bij", uvd[:, :, :2], inp_res)                                     
    d = (uvd[:, :, 2] - 0.5) * depth_range                                 

    if camera_mode == "persp":
        assert intr.dim() == 3, f"Unexpected dim, expect intr has shape (B, 3, 3), got {intr.shape}"
        root_joint_z = root_joint[:, -1].unsqueeze(-1)                 
        z = d * ref_bone_len + root_joint_z.expand_as(uvd[:, :, 2])                   
        ...
        camparam = torch.cat((fx, fy, cx, cy), dim=1)                 
        camparam = camparam.unsqueeze(1).expand(-1, uvd.shape[1], -1)                      
        xy_ = (uv - camparam[:, :, 2:4]) / camparam[:, :, :2]                      
        xy = xy_ * z.unsqueeze(-1).expand_as(uv)                      
        xyz = torch.cat((xy, z.unsqueeze(-1)), -1)                      
    elif camera_mode == "ortho":
        assert intr.dim() == 2, f"Unexpected dim, expect intr has shape (B, 3), got {intr.shape}"
        scale = intr[:, :1].unsqueeze(1)                    
        shift = intr[:, 1:].unsqueeze(1)                    
...
def batch_ref_bone_len(joint: Union[np.ndarray, torch.Tensor], ref_bone_link=None) -> Union[np.ndarray, torch.Tensor]:
    ...
    if torch.is_tensor(joint):
        ...
        for jid, nextjid in zip(ref_bone_link[:-1], ref_bone_link[1:]):
            bone += torch.norm(joint[:, jid, :] - joint[:, nextjid, :], dim=1, keepdim=True)          
    elif isinstance(joint, np.ndarray):
        bone = np.zeros((batch_size, 1))
        for jid, nextjid in zip(ref_bone_link[:-1], ref_bone_link[1:]):
            bone += np.linalg.norm((joint[:, jid, :] - joint[:, nextjid, :]), ord=2, axis=1, keepdims=True)          
    return bone
...
def batch_cam_intr_projection(batch_cam_intr, batch_joints, eps=1e-7):
    ...
    """
    res = (batch_cam_intr @ batch_joints.transpose(2, 3)).transpose(2, 3)                      
    xy = res[..., 0:2]
    z = res[..., 2:]
...
def generate_rand_transf(batch_size, r_angle=np.pi / 2, r_tsl=0.1, device="cpu"):
    ...
    angle = np.random.rand(batch_size) * (r_angle * 2) - r_angle                                      
    rot = rot * angle[:, None]

    rot = torch.from_numpy(rot).float().to(device)
    rot = aa_to_rotmat(rot)

    tsl = torch.rand(batch_size, 3).float().to(device) * r_tsl * 2 - r_tsl                                
...
def slerp_and_lerp_pose(matrix1, matrix2, t):
    ...
    slerp_rotation = slerp(t).as_matrix()             

    lerp_translation = []
    for i in t:
        lerp_t = (1 - i) * translation1 + i * translation2       
        lerp_translation.append(lerp_t)
    lerp_translation = np.stack(lerp_translation, axis=0)          

    result = np.eye(4)[None].repeat(len(t), axis=0)             
