...
class ManipData(Dataset, ABC):
    ...
    def compute_angular_velocity(r, time_delta: float, guassian_filter=True):
        ...
        diff_r = r[1:] @ r[:-1].transpose(-1, -2)                  
        diff_aa = rotmat_to_aa(diff_r).cpu().numpy()               
        diff_angle = np.linalg.norm(diff_aa, axis=-1)            
        diff_axis = diff_aa / (diff_angle[:, :, None] + 1e-8)               
        angular_velocity = diff_axis * diff_angle[:, :, None] / time_delta               
        angular_velocity = np.concatenate([angular_velocity, angular_velocity[-1:]], axis=0)             
        if guassian_filter:
    ...
    def load_retargeted_data(self, data, retargeted_data_path):
        if not os.path.exists(retargeted_data_path):
            ...
            if "vlm_mano_joints" in opt_params:
                update_dict["vlm_mano_joints"] = torch.tensor(opt_params["vlm_mano_joints"], device=self.device)
