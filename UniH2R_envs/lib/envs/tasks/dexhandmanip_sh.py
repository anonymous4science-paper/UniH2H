...
class DexHandManipRHEnv(VecTask):
    ...
    def __init__(
        ...
        *,
        ...
        self.success_rate_threshold = self.cfg["env"].get("success_rate_threshold", 0.3)
        self.w_expert_min = self.cfg["env"].get("w_expert_min", 0.2)
        self.w_expert_max = self.cfg["env"].get("w_expert_max", 1.0)
        self.w_success_max = self.cfg["env"].get("w_success_max", 3.0)
        self.transition_scale = self.cfg["env"].get("transition_scale", 10.0)
    ...
    def compute_reward(self, actions):
        ...
        max_length_list = max_length.int().tolist()
    ...
    def pre_physics_step(self, actions):
        ...
        if not self.headless:
            ...
            for env_id, env_ptr in enumerate(self.envs):
                ...
                if len(dexhand_joints) == self.dexhand.n_bodies:
                    dexhand_joint_tensor = torch.stack(dexhand_joints)
                    add_lines(self.viewer, env_ptr, dexhand_joint_tensor, color)
        ...
        base_action = torch.zeros_like(actions[:, :res_split_idx])                           
        ...
        dof_pos = 1.0 * base_joint_action + residual_joint_action
...
def compute_imitation_reward(
    ...
    if dexterous_hand == "franka_panda":
        if current_success_rate <= success_rate_threshold:
            ...
            contact_points = (finger_tip_distance < 0.02).sum(dim=-1)
            failed_execute = (
                (
                    (diff_obj_pos_dist > 0.02 / 0.343 * scale_factor**3)        
                    | (diff_obj_rot_angle.abs() / np.pi * 180 > 30 / 0.343 * scale_factor**3)        
        ...
        succeeded = (
            progress_buf + 1 + 3 >= torch.tensor(max_length, device=progress_buf.device)
        ) & ~failed_execute                                                                
        ...
        transition_factor = torch.clamp(torch.tensor((current_success_rate - success_rate_threshold) * transition_scale), 0, 1)
        transition_factor_float = float(transition_factor.item())
        w_expert = max(w_expert_min, w_expert_max * (1.0 - transition_factor_float))
        w_success = w_success_max * transition_factor_float
        
                         
        completion_bonus = torch.where(
            succeeded, 
            torch.ones_like(progress_buf) * 5.0,              
        ...
        reward_success = (
            6 * reward_obj_pos +                
            2 * reward_obj_rot +               
            2 * reward_finger_tip_force +      
            20 * completion_bonus                          
        )
        
                  
        reward_execute = w_expert * reward_execute_base + w_success * reward_success
        ...
        if current_success_rate <= success_rate_threshold:
            ...
            failed_execute = (
                (
                    ...
                    | (diff_ring_tip_pos_dist > 0.06 / 0.7 * scale_factor*2)
                    | (diff_level_1_pos_dist > 0.07 / 0.7 * scale_factor*2)
                    | (diff_level_2_pos_dist > 0.08 / 0.7 * scale_factor*2)
                    | (diff_obj_rot_angle.abs() / np.pi * 180 > 30 / 0.343 * scale_factor**3)        
