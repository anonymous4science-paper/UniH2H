...
def rescale_actions(low, high, action):
    ...
    if d.shape[0] * 2 == action.shape[1]:                           
        d = torch.cat([d, d], dim=0)
        m = torch.cat([m, m], dim=0)
    elif (d.shape[0]) * 2 + 3 * 2 == action.shape[1]:                                                
        dof_d = d.shape[0] // 2 - 6
        d = torch.cat(
            [
                d[:6],
    ...
    elif (d.shape[0]) * 2 + 3 == action.shape[1]:                                                     
    ...
    def run(self):
        ...
        while True:
            ...
            if self.save_rollouts:
                ...
                states_to_save = unstack_sequence_fields(states_to_save, batch_size=self.env.num_envs)                
                    ...
                    if prev_done_count > 0:
                        ...
                        for prev_done_idx in prev_done_indices:
                            ...
                            if len(rollouts[prev_done_idx]) < self.min_episode_length:
                                ...
                                rollout_to_save = stack_sequence_fields(rollout_to_save)                        
