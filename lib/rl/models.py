...
class ModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(BaseModelNetwork):
        ...
        def forward(self, input_dict):
            ...
            logstd = torch.nan_to_num(logstd, nan=0.0, posinf=2.0, neginf=-10.0)
...
class SepModelA2CContinuousLogStd(BaseModel):
    ...
    class Network(nn.Module):
        ...
        def forward(self, input_dict):
            ...
            lh_mu, lh_logstd, lh_value, lh_states = self.a2c_network.left_hand_net(lh_input_dict)          
            lh_logstd = torch.nan_to_num(lh_logstd, nan=0.0, posinf=2.0, neginf=-10.0)
            lh_logstd = torch.clamp(lh_logstd, min=-10.0, max=2.0)
            lh_sigma = torch.exp(lh_logstd)
            lh_sigma = torch.clamp_min(lh_sigma, 1e-6)
            ...
            rh_logstd = torch.nan_to_num(rh_logstd, nan=0.0, posinf=2.0, neginf=-10.0)
            rh_logstd = torch.clamp(rh_logstd, min=-10.0, max=2.0)
            rh_sigma = torch.exp(rh_logstd)
            rh_sigma = torch.clamp_min(rh_sigma, 1e-6)
            ...
            if is_train:
                ...
                rh_result = {
                    ...
                    "values": self.right_hand_base.denorm_value(rh_value),          
                ...
                lh_result = {
                    ...
                    "values": self.left_hand_base.denorm_value(lh_value),          
