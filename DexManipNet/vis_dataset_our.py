...
if __name__ == "__main__":
    ...
    data_item = dataset[args.idx]
    if "q_rh" in data_item:
        seq_len = len(data_item["q_rh"])
    elif "q_lh" in data_item:
        seq_len = len(data_item["q_lh"])
    elif "actions" in data_item:
        seq_len = len(data_item["actions"])
    elif "seq_len" in data_item:
        seq_len = data_item["seq_len"]
    else:
        seq_len = "unknown"
    cprint(f'sequence length: {seq_len}', "yellow")
